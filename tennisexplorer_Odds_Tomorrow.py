                ...
                for c in range(item + 1, tournament_idx_lst[i + 1], 2):
                    ...
                    if len(test) > 1:
                        tournament_dict[tournament_name + str(item)][
                            ...
                            + "False"
                            + "_"
                            + rows[c].find_all("td", class_="first time")[0].contents[0]
                        )
                    else:
                        tournament_dict[tournament_name + str(item)][
                            current_date
    ...
    for key, value in tournament_dict.items():
        ...
        for i, date in value.items():
            for match in date:
                ...
                new_df.to_sql(
                    "TomorrowsMatches", con=devengine, if_exists="append", index=False
                )
...
def Tomorrow():
    ...
    for x in range(1, 2):
        print(x)
        connection.execute(text("Delete FROM TomorrowsMatches"))
        connection.commit()
