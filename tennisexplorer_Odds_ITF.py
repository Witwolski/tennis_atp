...
def Main(url, current_date, suffix, check):
    ...
    for i, item in enumerate(tournament_idx_lst[:-1]):
        ...
        if "Futures" in tournament_name or "ITF" in tournament_name:
            tournament_url = (
            ...
            if not "cuck" in tournament_name:
                if not tournament_dict.get(tournament_name):
                    tournament_dict[tournament_name + "_" + str(item)] = {}
                    tournament_dict[tournament_name + "_" + str(item)][
                        current_date
                    ] = []
    ...
    for key, value in tournament_dict.items():
        ...
        for i, date in value.items():
            for match in date:
                ...
                new_df.to_sql(
                    "AllMatches_ITF", con=devengine, if_exists="append", index=False
                )


# for x in range(81,90):
for x in range(62, 365):
