...
logging.basicConfig(filename="error_results.log", level=logging.ERROR)
...
def Main(url, current_date, suffix, check):
    ...
    table = soup.find_all("table", {"class": "result"})[1]
    if check == 1:
        table = soup.find("table", {"class": "result"})
    ...
    for i, item in enumerate(tournament_idx_lst[:-1]):
        ...
        if (
            ...
            if (
                not "Futures" in tournament_name
                and not "Manama" in tournament_name
                and not "Ultimate" in tournament_name
            ):
                ...
                for c in range(item + 1, tournament_idx_lst[i + 1], 2):
                    test = rows[c].find_all("td", class_="course")
                    # print(len(test))
                    if len(test) > 1:
                        tournament_dict[tournament_name + str(item)][
                            current_date
                            ...
                            + court_type
                            + ":"
                            + rows[c].find_all("td", class_="course")[0].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="course")[1].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="result")[0].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="result")[1].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="score")[0].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="score")[1].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="score")[2].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="score")[3].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="score")[4].contents[0]
                            + "_"
                            + rows[c + 1].find_all("td", class_="score")[0].contents[0]
                            + "_"
                            + rows[c + 1].find_all("td", class_="score")[1].contents[0]
                            + "_"
                            + rows[c + 1].find_all("td", class_="score")[2].contents[0]
                            + "_"
                            + rows[c + 1].find_all("td", class_="score")[3].contents[0]
                            + "_"
                            + rows[c + 1].find_all("td", class_="score")[4].contents[0]
                        )
                    else:
                        tournament_dict[tournament_name + str(item)][
                            current_date
                            ...
                            + court_type
                            + ":"
                            + rows[c].find_all("td", class_="coursew")[0].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="course")[0].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="result")[0].contents[0]
                            + "_"
                            + rows[c + 1].find_all("td", class_="result")[0].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="score")[0].contents[0]
                            + "_"
                            + rows[c].find_all("td", class_="score")[1].contents[0]
...
with open("error_results.log", "r") as f:
    error_log = f.read()

if "ERROR" in error_log:
    send_email("Error in Prevous Days Data", error_log)
